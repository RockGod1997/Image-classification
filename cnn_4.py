...
def relabel(labels):
    for idx,item in enumerate(labels):
        if item < 58 :
            ...
            labels[idx] = 2     # small letters
        
        else :
            labels[idx] = 3    #capital letters
    return labels
def f_score(p,r):
  f = (2*p*r)/(p+r)
  return f    
...
for i in range(1):
              ...
              file1 = open('results_cnn_4.csv','a')
              ...
              model.fit_generator(train_generator, steps_per_epoch=actual_train_size//256, epochs=1,validation_data=test_generator, validation_steps=size//2560)
